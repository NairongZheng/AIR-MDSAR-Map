...
def basic_Block(input, out_filters, strides=(1, 1), with_conv_shortcut=False):      # 直接进行卷积
    x = conv3x3(input, out_filters, strides)
    x = BatchNormalization(axis=3)(x)
    x = Activation('relu')(x)

    x = conv3x3(x, out_filters)
    x = BatchNormalization(axis=3)(x)                       # 这里不使用relu是为了避免加和之后的特征都是正的，保证特征的多样性

    if with_conv_shortcut:
        residual = Conv2D(out_filters, 1, strides=strides, use_bias=False, kernel_initializer='he_normal')(input)
        residual = BatchNormalization(axis=3)(residual)     # 同样这里也不进行relu
        x = add([x, residual])
    else:
        x = add([x, input])
...
def bottleneck_Block(input, out_filters, strides=(1, 1), with_conv_shortcut=False):     # 因为比较深才用这个，所以先降维，最后升维
    expansion = 4
    ...
    if with_conv_shortcut:
        residual = Conv2D(out_filters, 1, strides=strides, use_bias=False, kernel_initializer='he_normal')(input)
        residual = BatchNormalization(axis=3)(residual)     # 这里也不进行relu
        x = add([x, residual])
    else:
        x = add([x, input])
...
def seg_hrnet(batch_size, height, width, channel, classes):
    ...
    x = transition_layer1(x)        # 把第一阶段的分两条，一个继续卷积保持高分辨率，一个下采样变成1/2
    x0 = make_branch1_0(x[0])
    x1 = make_branch1_1(x[1])
    x = fuse_layer1([x0, x1])

    x = transition_layer2(x)        # 把第二阶段的两条分成三条，两个都继续卷积，一个下采样再降1/2
    x0 = make_branch2_0(x[0])
    x1 = make_branch2_1(x[1])
    x2 = make_branch2_2(x[2])
    x = fuse_layer2([x0, x1, x2])

    x = transition_layer3(x)        # 把第三阶段的三条分成四条，三个都继续卷积，一个下采样再降1/2
