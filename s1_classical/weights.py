...
def _xception_block(inputs, depth_list, prefix, skip_connection_type, stride,
    ...
    for i in range(3):
        ...
        if i == 1:
            skip = residual
    if skip_connection_type == 'conv':      # 对input进行卷积之后作为残差边
        shortcut = _conv2d_same(inputs, depth_list[-1], prefix + '_shortcut', kernel_size=1, stride=stride)
        shortcut = BatchNormalization(name=prefix + '_shortcut_BN')(shortcut)
        outputs = layers.add([residual, shortcut])
    elif skip_connection_type == 'sum':     # 直接把input作为残差边
        outputs = layers.add([residual, inputs])
    elif skip_connection_type == 'none':    # 没有残差边
        outputs = residual
    if return_skip:
        return outputs, skip
    else:
...
def Deeplabv3(input_shape=(None, None, 3), classes=8, OS=16, activation='softmax', drop_pro=0.5, middle_repeat=8, tiao=1):
    ...
    if tiao:
        x = Concatenate()([x_attention, b4, b0, b1, b2, b3])
    else:
        x = Concatenate()([b4, b0, b1, b2, b3])
    # 1X1 conv
    if tiao:
        x = Conv2D(256, (1, 1), padding='same', use_bias=False, name='concat_projection_PSP')(x)
        x = BatchNormalization(name='concat_projection_PSP_BN', epsilon=EPS)(x)
        x = Activation('relu')(x)
    else:
    ...
    if classes == 2:
        ...
        last_layer_name = 'custom_logits_semantic_7'
    else:
...
if __name__ == '__main__':
    ...
    # model = Deeplabv3(input_shape=(None, None, 3), classes=8)
    model = Deeplabv3(input_shape=(256, 256, 3), classes=8)
    # model.load_weights(r'E:\pretrain_model\deeplabv3_xception_tf_dim_ordering_tf_kernels.h5', by_name=True)
    model.summary()
