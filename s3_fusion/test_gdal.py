...
class TestDataLoader():
    ...
    def make_dataset(self, img_path, height=256, width=256, stride=256):
        (image, h, w, band_num) = self.read_hyper(img_path)
        img = self.hyper2numpy(image, h, w, band_num)
        ...
        samples = np.zeros((n_row * n_col, height, width, band_num), dtype=np.uint8)
        ...
        for m in range(n_row):
            ...
            for n in range(n_col):
                ...
                # img_mn = img_mn.transpose((2, 0, 1))
                # img_mn = np.expand_dims(img_mn, axis=0).repeat(3, axis=0)
                samples[K, :, :, :] = img_mn
                K += 1
    ...
    def __getitem__(self, index):
        ...
        # 去看实验记录，看看不同的模型这边对应选哪个XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        # sample = np.expand_dims(sample, axis=0)
        sample = np.expand_dims(sample, axis=0).repeat(3, axis=0)
    ...
    def inference_flip(self, model, image, gpu, flip=False):
        ...
        if flip:
            flip_img = image.cpu().numpy()[:, :, :, :, ::-1]           # 水平翻转XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
def save_pred_xiongan(preds, Height, Width, name, args):
    label_mapping = {"label1":[0, 0, 0], "label2":[255, 127, 80], "label3":[0, 74, 0], 
                "label4":[0, 0, 255], "label5":[255, 255, 0], "label6":[0, 255, 255], 
                "label7":[189, 41, 181], "label8":[139, 139, 0], "label9":[0, 158, 0], 
                "label10":[145, 0, 148], "label11":[222, 184, 135], "label12":[218, 140, 214], 
                "label13":[139, 0, 0], "label14":[0, 255, 0], "label15":[255, 182, 193], 
                "label16":[205, 0, 0], "label17":[46, 139, 87], "label18":[0, 0, 139], 
                "label19":[255, 165, 0], "label20":[85, 26, 139]}
    for i, (k, v) in enumerate(label_mapping.items()):
        globals()[k] = preds == i                       # 项目的数据是白边，标了之后自动变成0，所以要+1
    h = preds.shape[0]
    w = preds.shape[1]
    del preds
    rgb = np.zeros((h, w, 3), dtype=np.uint8)
    for i, (k, v) in enumerate(label_mapping.items()):
        rgb[:, :, 0] += (globals()[k] * v[0]).astype(np.uint8)
        rgb[:, :, 1] += (globals()[k] * v[1]).astype(np.uint8)
        rgb[:, :, 2] += (globals()[k] * v[2]).astype(np.uint8)
    rgb = rgb[:Height, :Width, :]
    save_img = Image.fromarray(np.uint8(rgb))
    save_img.save(os.path.join(args.save_path, 'pred_' + name + '.png'))
...
def parse_args():
    ...
    parser.add_argument('--weights_path', help='the path of test model', default='/emwuser/znr/code/hyper_sar/outputs/20220610_3d_hrnet_epoch200_lr1e-2_MS_pretrain_imagenet/hyper_sar/3dhrnet_w18_pretrain_copy_2d')
    parser.add_argument('--weights_name', help='the name of test model weight', default='hrnet_3d_epoch201_tr_fwiou_0.82906_tr_OA_0.8972_val_fwiou_0.65740_val_OA_0.7816.pth')
    parser.add_argument('--save_path', help='the path of save file', default='/emwuser/znr/data/hyper_sar/test/pre/try')
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(testloader)):
            ...
            if args.multi_scaled_test:
                ...
                pred = test_dataset.inference_flip(model, image, args.gpu, flip=True)      # XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
        ...
        # save_pred_xiongan(pred_np, int(Height.numpy()[0]), int(Width.numpy()[0]), name, args)
...
def main_worker(gpu, args):
    ...
    # 这边要选择模型的XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
