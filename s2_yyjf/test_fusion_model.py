...
def parse_args():
    ...
    parser.add_argument('--data', help='the path of testing data', default='/emwuser/znr/data/yyjf/Ka/test/img')
    ...
    parser.add_argument('--used_models', help='the used models band', default=['C', 'Ka', 'S'])
...
def test_model(test_dataset, testloader, used_model, name, args):
    """
        测试模型
    """
    n_pred = []
    with torch.no_grad():
        for i, batch in enumerate(tqdm(testloader)):
            ...
            pred = torch.zeros((1, args.classes, image.shape[-2], image.shape[-1]))
            pred = pred.cuda(args.gpu)
            for i, model in enumerate(used_model):
                if i == 1:      # ka波段
                    rate = 1
                else:
                    rate = 0
                ...
                if args.multi_scaled_test:
                    ...
                    tmp = test_dataset.multi_scale_inference_resize(model, image, args.gpu, scales, flip=True)
                    tmp = tmp / torch.max(tmp)
                    pred += tmp
                else:
                    tmp = test_dataset.inference_flip(model, image, args.gpu, flip=True)
                    tmp = tmp / torch.max(tmp)
                    pred += tmp
...
def load_all_used_models(model, model_path):
    """
        加载所有用来测试的模型
    """
    pretrained_dict = torch.load(model_path)        # 加载训练好的模型权重
...
def main_worker(gpu, args):
    ...
    if args.used_models is not None:
        print('use {} model to test'.format(args.used_models))
        for bands in args.used_models:
            if bands == 'C':
                model_c_path = os.path.join(args.weights_path, 'C/yyjf_C/hrnet_w18_train_512_1e-2_C', 'hrnet_w18_epoch196_fwiou_0.94995_OA_0.9735.pth')
                model_c = eval(config.MODEL.NAME + '.get_seg_model')(config)
                model_c = load_all_used_models(model_c, model_c_path)
                # model_c.cuda(args.gpu)
                used_model.append(model_c)
                del(model_c)
                print('load model c done!')
            elif bands == 'Ka':
                model_ka_path = os.path.join(args.weights_path, 'Ka/yyjf_Ka/hrnet_w18_train_512_1e-2_Ka', 'hrnet_w18_epoch199_fwiou_0.95115_OA_0.9737.pth')
                model_ka = eval(config.MODEL.NAME + '.get_seg_model')(config)
                model_ka = load_all_used_models(model_ka, model_ka_path)
                # model_ka.cuda(args.gpu)
                used_model.append(model_ka)
                del(model_ka)
                print('load model ka done!')
            elif bands == 'S':
                model_s_path = os.path.join(args.weights_path, 'S/yyjf_S/hrnet_w18_train_512_1e-2_S', 'hrnet_w18_epoch188_fwiou_0.93236_OA_0.9637.pth')
                model_s = eval(config.MODEL.NAME + '.get_seg_model')(config)
                model_s = load_all_used_models(model_s, model_s_path)
                # model_s.cuda(args.gpu)
                used_model.append(model_s)
                del(model_s)
                print('load model s done!')
        ...
        for i in range(len(names)):
            ...
            name = names[i]
            test_model(test_dataset, test_loader, used_model, name, args)
            pass
