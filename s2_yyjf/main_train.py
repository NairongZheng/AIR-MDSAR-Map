...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='train segmentation network (hrnet)')
    parser.add_argument('--cfg', help='the path of config file', default='')
    ...
    # training options
    parser.add_argument('--batch_size', help='mini-batch size', default=16)
    ...
    for epoch in range(0, end_epoch):
        ...
        if rank == 0:
            if train_fw_iou > best_train_fwiou:
                ...
                if train_fw_iou > 0.8:
                    save_name = 'hrnet_w18' + '_epoch' + str(epoch + 1) + '_tr_fwiou_' + str(train_fw_iou)[:7] + '_tr_OA_' + str(train_MOA)[:6]\
